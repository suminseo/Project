...
@WebServlet("/QT/admin/market/delete")
public class AdminMarDeleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	private GoodsService service = new GoodsService();
	
    public AdminMarDeleteServlet() {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (loginMember != null && loginMember.getRole().equals("ROLE_ADMIN")) {
			...
			if (result > 0) {
				...
				request.setAttribute("location", "/QT/admin/market");
			} else {
