...
public class BoardDao {
	...
	public List<GoodsReply> getRepliesByNo(Connection connection, int boardNo) {
		...
		try {
			query = 
				  "SELECT C.G_COMMENT_NO, C.G_PRODUCT, C.G_BOARD_CONTENT, M.USER_ID, C.G_COMMENT_CREATED, C.G_COMMENT_MODIFIED "
				+ "FROM GOODS_COMMENT C"
				+ "JOIN QT_USER M ON(C.G_WRITER_ID = M.USER_ID) "
				+ "WHERE G_PRODUCT=? "
				+ "ORDER BY C.G_COMMENT_NO DESC";
