...
public class BoardFileDownServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	// 2. 물리적인 저장소에 저장되어 있는 파일을 메모리로 가져온다.
    	...
    	// 6. 응답 메시지 작성(html을 보내는게 아니라 file을 보내는 것이기 때문에)
    	// octet-stream : 모든 종류의 이진 데이터, 무슨 파일 타입인지 모를 때 이 타입을 사용한다.)
    	response.setContentType("application/octet-stream");
    	
    	// 파일 링크를 클릭했을 때 다운로드 화면이 출력되게 하는 부분
