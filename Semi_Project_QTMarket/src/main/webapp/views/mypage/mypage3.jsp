      ...
      <section class="main__user">
        ...
        <div class="main__user__body">
          ...
          <div class="main__user__body__product">
			...
			<c:if test="${ !empty list }">
				...
				<c:forEach var="wish" items="${ list }">
					<tr>
						...
						<td>
							<a href="${ pageContext.request.contextPath }/board/view?no=${ wish.no }">
								${ wish.title }
							</a>
						</td>
						<td>${ wish.price }</td>
						<td>${ wish.createDate }</td>
						<td>${ wish.readCount }</td>
						<td>
            ...
            <div id="pageBar">
			<!-- 맨 처음으로 -->
			<button class="setbutton" onclick="location.href='${ pageContext.request.contextPath }/QT/mywish?page=1'">&lt;&lt;</button>

			<!-- 이전 페이지로 -->
			<button class="movebutton" onclick="location.href='${ pageContext.request.contextPath }/QT/mywish?page=${ pageInfo.prevPage }'">&lt;</button>
			...
			<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" varStatus="status">
				...
				<c:if test="${ status.current != pageInfo.currentPage }">				
					<button class="pagebutton" onclick="location.href='${ pageContext.request.contextPath }/QT/mywish?page=${ status.current }'">${ status.current }</button>
				</c:if>
			</c:forEach>

			<!-- 다음 페이지로 -->
			<button class="movebutton" onclick="location.href='${ pageContext.request.contextPath }/QT/mywish?page=${ pageInfo.nextPage }'">&gt;</button>

			<!-- 맨 끝으로 -->
			<button class="setbutton" onclick="location.href='${ pageContext.request.contextPath }/QT/mywish?page=${ pageInfo.maxPage }'">&gt;&gt;</button>
			</div>
